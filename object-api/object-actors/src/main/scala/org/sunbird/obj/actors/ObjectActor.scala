...
import org.sunbird.obj.util.ObjectConstants
      ...
      if(transitionProps.contains(operation)){
        ...
        DataNode.read(readReq).map(node => {
          if (!fromStatus.contains(node.getMetadata.get("status").toString)) {
            throw new ClientException(ObjectConstants.ERR_CONTENT_NOT_DRAFT, "Transition not allowed! "+ schemaName.capitalize +" Object status should be one of :" + fromStatus.toString())
          }
